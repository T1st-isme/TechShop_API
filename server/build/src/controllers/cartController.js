...
export const addItemToCart = asyncHandler(async (req, res) => {
  ...
  if (cart) {
    ...
    for (const cartItem of req.body.cartItems) {
      ...
      if (item) {
        ...
        update = {
          ...
          $set: {
            "cartItems.$.price": product.price * (item.quantity + cartItem.quantity)
          } // update the price
        };
      } else {
        condition = {
    ...
    const cartItemsWithPrice = await Promise.all(req.body.cartItems.map(async item => {
...
export const updateCart = asyncHandler(async (req, res) => {
  const {
    productId,
    quantity
  } = req.body; // Ensure these fields are present in the request body
...
export const getCartItems = asyncHandler(async (req, res) => {
  const cart = await Cart.findOne({
    user: req.user._id
  }).populate("cartItems.product", "_id name price proImg").exec();
  if (!cart) {
  ...
  res.status(200).json({
    ...
    total_price: (cart.cartItems.reduce((total, item) => total + parseFloat(item.product.price * item.quantity),
    //total price with quantity
    0) * 1000000).toFixed(2),
    total_quantity: cart.cartItems.reduce((total, item) => total + item.quantity, 0),
    cartItems
  });
});
