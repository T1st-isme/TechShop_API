...
export const addItemToWishList = asyncHandler(async (req, res) => {
  ...
  if (wishList) {
    const itemExists = wishList.wishListItems.find(item => item.product.toString() === product._id.toString());
    if (itemExists) {
      return res.status(400).json({
...
export const getWishListItems = asyncHandler(async (req, res) => {
  const wishList = await WishList.findOne({
    user: req.user._id
  }).populate("wishListItems.product", "_id name price proImg slug");
  if (!wishList) {
...
export const removeItemFromWishList = asyncHandler(async (req, res) => {
  ...
  wishList.wishListItems = wishList.wishListItems.filter(item => item.product.toString() !== req.body.productId);
  await wishList.save();
  res.status(200).json({
    success: true,
    wishList
